module.exports = (m) => {
   ...
   if (setting) {
      ...
      if (!('owners' in setting)) setting.owners = ['6282160268332']
      if (!('cover' in setting)) setting.cover = 'https://www6.flamingtext.com/net-fu/proxy_form.cgi?&imageoutput=true&script=sketch-name&doScale=true&scaleWidth=1200&scaleHeight=800&fontsize=150&fillTextType=1&fillTextPattern=Warning&text=Onee%20BotZ'
      ...
      if (!('footer' in setting)) setting.footer = '𝙾𝚗𝚎 𝙱𝚘𝚝𝚣-𝙼𝙳 𝚋𝚢 𝙳𝚊𝚟𝚒𝚍 ッ'
   } else {
      global.db.setting = {
         autobackup: false,
         backupTime: 36000,
         ...
         prefix: ['.', '#', '!', '/'],
         onlyprefix: '+',
         owners: ['6282160268332'],
         cover: 'https://www6.flamingtext.com/net-fu/proxy_form.cgi?&imageoutput=true&script=sketch-name&doScale=true&scaleWidth=1200&scaleHeight=800&fontsize=150&fillTextType=1&fillTextPattern=Warning&text=Onee%20BotZ',
         header: 'COLOUR YOUR LIFE',
         msg: 'I am a Whatsapp BOT, use a bot in private chat so that bots can respond quickly.',
         footer: '𝙾𝚗𝚎 𝙱𝚘𝚝𝚣-𝙼𝙳 𝚋𝚢 𝙳𝚊𝚟𝚒𝚍 ッ'
      }
   }
}